...
def append_sig_suffix(s: str) -> str:
    return s + _sig_suffix


def path_append_sig_suffix(path: Path) -> Path:
    return common.path_append(path, _sig_suffix)
    ...
    def _get_gpg(self) -> Optional[gnupg.GPG]:
        if self._gpg_home is None:
            ...
            try:
                ...
                common.eprint(
                    "** Warning: GPG setup failed; "
                    "signature verification disabled"
                )
        return self._gpg

    def verify(self, path: Path, sig_path: Path) -> None:
        if not path.is_file():
            raise error.MissingFileError(str(path))
        if not sig_path.is_file():
            raise error.MissingFileError(str(sig_path))
        gpg = self._get_gpg()
        if gpg is not None:
            with sig_path.open("rb") as sig:
                verified = gpg.verify_file(sig, str(path))
                if not verified:
                    raise error.IntegrityError(
                        str(path), sig_name=str(sig_path)
                    )
